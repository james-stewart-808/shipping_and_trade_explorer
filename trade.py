...
st.markdown(
    " - It's important to note that UNCTAD's Trade-and-Transport Database is fundamentally \
    based on Comtrade data."
)
st.markdown(
    " - The quality of data hosted on Comtrade is highly variable depending on the country and region."
)
st.markdown(
    " - Presented below is the Comtrade contribution record for the selected country of interest over \
    the decade from 2014-23."
)
st.markdown(
    " - Please bear this information in-mind when exploring the trade data - merchandise trade statistics \
    will be more reliable for countries that have provided their data often over recent years."
)
...
def com_cont_aggregator(df, aggregator):
    df_group_cols = {
        "iso_code":"count", 
        "2014":"sum", "2015":"sum", "2016":"sum", "2017":"sum", "2018":"sum", 
        "2019":"sum", "2020":"sum", "2021":"sum", "2022":"sum", "2023":"sum"
    }
    df_grouped = df.groupby(aggregator).agg(df_group_cols).reset_index()
    df_grouped["Contributions"] = df_grouped["2014"] + df_grouped["2015"] + df_grouped["2016"] + \
        df_grouped["2017"] + df_grouped["2018"] + df_grouped["2019"] + df_grouped["2020"] + \
        df_grouped["2021"] + df_grouped["2022"] + df_grouped["2023"]
...
contributed = sum([by_country_contrib_record_v1[x].value_counts().values[0] for x in \
...
st.write(
    "All countries for which data is available: {0} contributions over the last 10 years out of a possible {1}, \
    representing an overall record of {2}%".format(
        contributed, total, contributed_frac)
)
...
if comtrade_rec_cou.shape[0] == 0:
    ...
    st.write(
        "The contribution record of {0} over the last 10 years is shown above, representing an average contribution rate \
        of {1}%. Further information on Comtrade Contribution Rates is provided below.".format(
            st.session_state.iso_country, int(100 * comtrade_rec_cou.iloc[:, -10:].sum().sum() / 10)
        )
    )
